...
template <typename HANDLE_T>
static inline uintptr_t HandleToUintPtr(HANDLE_T h) {
    return CastToUintPtr<HANDLE_T>(h);
}
...
#if defined(__CHERI_PURE_CAPABILITY__)
        return FormatHandle(VkHandleInfo<T>::Typename(), HandleToUintPtr(handle));
#else // defined(__CHERI_PURE_CAPABILITY__)
...
#if defined(__CHERI_PURE_CAPABILITY__)
    debug_report->RemoveDebugUtilsCallback(CastToUintPtr(callback));
#else // defined(__CHERI_PURE_CAPABILITY__)
